<%@page import="proj21_admin.dto.review.ReviewDTO"%>
		...
		<div class="row">
			<div class="col-12">
				<div class="card">
					...
					<center>
						...
						<form action="${contextPath }/admin/review/listReviews"
							...
							<table style="width: 100%;">
								<tr>
									<td colspan="2">&nbsp;&nbsp;</td>
									<td colspan="5" class="pleft">멤버 ID : <input
										style="margin-left: 14.3px;" type="text" name="memberId"
										placeholder="멤버 ID를 입력하세요" id="memberId" />
									</td>
								</tr>
								<tr>
									<td colspan="2">&nbsp;&nbsp;</td>
									<td colspan="5" class="pleft">제품 번호 : <input type="text"
										name="proNum" placeholder="제품 번호를 입력하세요" id="proNum" />
									</td>
								</tr>
					...
					<hr>
					<form method="post"
						action="${contextPath }/admin/review/deleteReviews"
						id="frm_delete">
						<table border="1"
							style="border-color: #ddd #ddd #848484 #ddd; width: 100%;">
									...
									<c:forEach var="reviews" items="${reviewsList}">
										<tr style="text-align: center;">
											<td><input type="checkbox" name="checkRow"
												value="${reviews.reviewNum }" /></td>
											<td><a
												href="${contextPath }/productDetail?proNum=${reviews.proNum}">${reviews.memberId }</a></td>
											<td><a
												href="${contextPath }/productDetail?proNum=${reviews.proNum}">${reviews.proNum }</a></td>
											<td><a
												href="${contextPath }/productDetail?proNum=${reviews.proNum}">${reviews.proName.proName }</a></td>
											...
											<td><a
												href="${contextPath }/productDetail?proNum=${reviews.proNum}">${reviews.reviewContent }</a></td>
											<td><a
												href="${contextPath }/productDetail?proNum=${reviews.proNum}">${reviews.reviewStar }</a>
												점</td>
											<td><a
												href="${contextPath }/productDetail?proNum=${reviews.proNum}">${reviews.reviewDate }</a></td>
										</tr>
									</c:forEach>
								</c:when>
							</c:choose>
							<tr>
								<td colspan="8" style="text-align: left;"><input
						...
						<c:if test="${selectedReviews !=null}">
							<c:choose>
								<c:when test="${selectedReviews > 200 }">
									<c:forEach var="page" begin="1" end="10" step="1">
										...
										<c:if test="${section >1 && page==1 }">
											...
											<a
												href="${contextPath }/admin/review/listReviews?section=${section-1}&pageNum=${(section-1)*20+1}">%nbsp;
												pre </a>
										</c:if>

										<a
											href="${contextPath }/admin/review/listReviews?section=${section}&pageNum=${page}">${(section-1)*20 + page }</a>

										<c:if test="${page==20} }">

											<!--페이지수가 10일때 다음으로 next나오게 하기  -->
											<a
												href="${contextPath }/admin/review/listReviews?section=${section+1}&pageNum=${section*10+1}">&nbsp;
												next</a>
										</c:if>
									</c:forEach>
								</c:when>
								...
								<c:when test="${selectedReviews < 200 }">
									<c:forEach var="page" begin="1" end="${selectedReviews/20+1 }"
										step="1">
										<c:choose>
											<c:when test="${page==pageNum }">
												<a class="sel-page"
													style="color: red; font-size: 40px; padding-left: 8px;"
